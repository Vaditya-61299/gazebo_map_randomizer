...
'''def create_obstacle(num_obst):
    ...
    for i in range(min(num_obst, len(object_pool))):  # Don't exceed model count
        _name = f"obstacle_{i}"
        scale = get_scale()
        pose = get_pose()

        selected_model, mesh_name, extension = object_pool[i]
        ...
        obstacles.append((_name, pose))  # For world file
...
"""
    ...
    with open(os.path.join(world_folder, "world.world"), 'w') as f:
